...
$bookids = optional_param_array ('check_book', array(), PARAM_INT);
//print_object($bookids);
...
// Launch the SQL quey.
$bookresults = search( $query, $course,  $bookids, $start, $countentries);
...
if (!empty($bookresults)) {
    // Print header
    echo '<span style="float: left">'.$strresults.' <b>'.($startindex+1).'</b> - <b>'.$endindex.'</b> '.$ofabout.'<b> '.$countresults.' </b>'.$for.'<b> "'.s($query).'" </b> from<b> ' .  count($bookids) . ' </b>selected book(s)</b></span><br /><br />';
    echo $page_bar;
    // Prepare each entry (hilight, footer...)
    echo '<table id="search_results"><tr><th style="width=70%;">Book</th><th></th><th style="width=70%;">Chapter</th></tr>'; //amendment ul to table
    foreach ($bookresults as $entry) {
        $book = $DB->get_record('book', array('id' => $entry->bookid));
        $cm = get_coursemodule_from_instance("book", $book->id, $course->id);

        //To show where each entry belongs to
        $result = "<tr><td><a href=\"$CFG->wwwroot/mod/book/view.php?id=$cm->id\">".format_string($book->name,true)."</a></td><td> &nbsp;&raquo;&nbsp; </td><td style='padding-left20px;'><a href=\"$CFG->wwwroot/mod/book/view.php?id=$cm->id&amp;chapterid=$entry->id\">".format_string($entry->title,true)."</a></td></tr>";
        echo $result;
    }
    echo '</table>';
    echo $page_bar;
